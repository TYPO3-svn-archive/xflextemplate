...
*
*  This copyright notice MUST APPEAR in anll copies of the script!
***************************************************************/
...
class tx_xflextemplate_tceforms	{
	...
	*/
	...
	//debug($row,'tceforms');
		if($row['xtemplate']){ //if xtemplate is not set none to do
		//fetch from db the xml which will create the forms
			...
			if(is_array($flexFields)){ //if flexdata is an array data will be put into flexdata array
				...
				foreach($flexFields as $key=>$obj){
					if($GLOBALS['TCA']['tt_content']['columns'][$key]['config']['internal_type']=='db' && $obj){
						//$row[$key]=$this->translatepages($obj);
						$row[$key]=$obj;
					}
					else{
						$row[$key]=$obj;
	 ...
	 * @ver 1.0.0
	 */
	function translatepages($list){
		$list=($list)?$list:'0';
		// when form is not submitted in the pages field there are only ids
		if (!strstr($list,'|')){ // if inside string is present "|" means list of pages is already correctly formatted, otherwise extract pages
			$res=$GLOBALS['TYPO3_DB']->exec_SELECTquery('title,uid','pages','uid in ('.$list.')');
			while($dbrow=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)){
				$outString[]='pages_'.$dbrow['uid'].'|'.$dbrow['title']; // create array of uid|title
			}
